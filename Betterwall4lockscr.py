        ...
        if self.random_walls == False:
            if self.get_time_period() == "Sunrise" or self.get_time_period() == "Sunset":
                ...
                with open('log_lock.txt','a') as log_lock:
                    log_lock.write(f'Updated the wall {self.Msunran_image} (Msunset) on {datetime.datetime.now()}  \n')
                ...
                if ksfja == 0:
                    ...
                    self.Wallpaper_Changed = True
                    with open('log_lock.txt','a') as log_lock:
                        log_lock.write(f'Updated the wall {self.MlightThemeran_image} (Mlight) on {datetime.datetime.now()} \n')
                elif ksfja == 1:
                    os.system('betterlockscreen -u '+'/home/Akash/Pictures/dt/wallpapers/Light-themes/'+self.lightThemeran_image)
                    self.Wallpaper_Changed = True
                    with open('log_lock.txt','a') as log_lock:
                        log_lock.write(f'Updated the wall {self.lightThemeran_image} (Light) on {datetime.datetime.now()} \n')
                ...
                if ksfja == 0:
                    ...
                    self.Wallpaper_Changed = True
                    with open('log_lock.txt','a') as log_lock:
                        log_lock.write(f'Updated the wall {self.MdarkThemeran_image} (Mdark) on {datetime.datetime.now()} \n')
                elif ksfja == 1:
                    os.system('betterlockscreen -u '+'/home/Akash/Pictures/dt/wallpapers/Dark-themes/'+self.darkThemeran_image)
                    self.Wallpaper_Changed = True
                    with open('log_lock.txt','a') as log_lock:
                        log_lock.write(f'Updated the wall {self.darkThemeran_image} (Dark) on {datetime.datetime.now()} \n')
